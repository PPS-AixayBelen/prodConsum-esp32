...
int isPos(rdp_o *rdp, int *index)
{
    ...
    {
        if (DEBUG)
            Serial.print("vector de disparo vacio o insensibilizado\n");
    ...
    for (int n = 0; n < PLACES; n++) // Si algun numero del nuevo vector de marcado es negativo, no puedo dispararla
    {
        ...
        {
            if (DEBUG)
                Serial.print("la transicion no se puede disparar, marcado resultante negativo\n");
            aux.v_methods->free_vector(&aux);
    ...
    if (DEBUG)
    {
        Serial.print("Nuevo sensiblizado : ");
